...
if text_col is None:
    st.markdown("""
    <div class="empty-state">
        <div class="empty-title">📝 No review text found</div>
        <div class="empty-text">
            This dataset does not contain review or text columns for NLP analysis.
            Try another dataset with customer reviews or product descriptions.
        </div>
    </div>
    """, unsafe_allow_html=True)

    if st.button("Go to Overview", key="go_overview_no_text"):
        st.switch_page("pages/1_Overview.py")
    st.stop()

st.markdown("""
<div class="hero-card">
    <div class="hero-badge">NLP Insights</div>
    <div class="hero-title">🧠 Text & Review Intelligence</div>
    <div class="hero-subtitle">
        Explore sentiment, keywords, topic clusters, and customer language patterns.
    </div>
    <div class="hero-muted">
        This page transforms unstructured review text into actionable insight using NLP techniques.
    </div>
</div>
""", unsafe_allow_html=True)

card_open()
section_title("📊 NLP Summary", "High-level indicators from the text dataset.")
n1, n2, n3 = st.columns(3)
n1.metric("Total Reviews", len(df_raw))
n2.metric(
    "Average Review Length",
    f"{df_raw['review_length'].mean():.1f} words" if "review_length" in df_raw.columns else "N/A"
)
n3.metric(
    "Average Rating",
    f"{df_raw['rating'].mean():.2f}" if "rating" in df_raw.columns else "N/A"
)
card_close()

if "sentiment_label" in df_raw.columns:
    card_open()
    section_title("🙂 Sentiment Summary", "Distribution of positive, neutral, and negative sentiment labels.")
...
card_open()
section_title("🔑 TF-IDF Keywords", "Most important words and phrases based on TF-IDF weighting.")
...
card_open()
section_title("🧩 Topic Clusters", "Group reviews into interpretable topic clusters.")
cluster_count = st.slider("Number of topic clusters", min_value=2, max_value=6, value=3)

clustered_df, top_terms_per_cluster, _ = cluster_reviews(
    df_raw["clean_text"],
...
if clustered_df is not None:
    ...
    for cluster_id, terms in top_terms_per_cluster.items():
        st.markdown(
            f"<div class='info-card'><strong>Cluster {cluster_id}:</strong> {', '.join(terms)}</div>",
            unsafe_allow_html=True
        )
else:
...
card_open()
section_title("➕ / ➖ Positive vs Negative Keywords", "Compare vocabulary used in positive and negative reviews.")
...
if "sentiment_label" in df_raw.columns:
    ...
    with c1:
        ...
        if pos_words:
            st.dataframe(pd.DataFrame(pos_words, columns=["Word", "Count"]), width="stretch")
        else:
    ...
    with c2:
        ...
        if neg_words:
            st.dataframe(pd.DataFrame(neg_words, columns=["Word", "Count"]), width="stretch")
        else:
            st.info("No strong negative keywords found.")
else:
    st.info("Sentiment labels are not available for this dataset.")
card_close()

card_open()
section_title("🔍 Search Reviews", "Search the review corpus by keyword.")
...
if query:
    ...
    if matched.empty:
        ...
        if title_col is not None and title_col in matched.columns:
            st.dataframe(matched[[title_col, text_col]].head(20), width="stretch")
        else:
            st.dataframe(matched[[text_col]].head(20), width="stretch")
card_close()
